# Enterprise EMR Appointment Management System — Backend

A production-oriented Node.js / Express / MongoDB (Mongoose) backend for an
Enterprise EMR Appointment Management module: authentication, RBAC, doctor
schedule configuration, dynamic slot generation, concurrency-safe appointment
booking, search/filter/pagination, audit logging, and real-time updates via
Socket.IO.
...
All business rules — status transitions, slot generation, double-booking
prevention — live in `services/`, so they're independently testable and
reusable outside the HTTP layer if needed (e.g. a future cron job or CLI).
